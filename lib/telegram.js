...
/*
 * The card NAMES THE NUMBER.
 *
 * It did not before, and that caused a real problem: a truncated number
 * ("2541074636" instead of "254107463687") was accepted, a code was issued for
 * it, and there was nothing on screen to reveal the mistake. Echoing the number
 * back makes a typo obvious before the code is used, and it is the only moment
 * the user can still catch it cheaply.
 */
function pairingCard(code, phone) {
    return [
        '╭────────────────────╮',
        '│ 🔐 PAIRING CODE',
        '│',
        ...
        '╰────────────────────╯',
        '',
        `⚠️ Enter this code on +${phone}. If that is not your number,`,
        'send /cancel and start again with /pair — the code will not work on any other number.',
        '',
...
async function pairNow(userId, phone, confirmed) {
    ...
    try {
        ...
        await sendMessage(userId, pairingCard(result.code, phone))
    } catch (error) {
        entry.state = 'idle'
