...
function sessionSettings(conn) {
  ...
  if (!config.sessionSettings[key] || typeof config.sessionSettings[key] !== 'object' || Array.isArray(config.sessionSettings[key])) {
    config.sessionSettings[key] = { antidelete: false, antidelete2: false, antidelete3: '', antidelete4: '' }
  }
  const s = config.sessionSettings[key]
  // Migrate stores written before antidelete4 existed.
  if (typeof s.antidelete4 !== 'string') s.antidelete4 = ''
  return s
}
...
/**
 * Every mode is read FRESH from the persisted store on each call. Nothing is
 * cached in a module variable, so `off` takes effect immediately and stays off
 * across a restart - there is no in-memory flag that can drift out of sync with
 * the file.
 */
function getSettings(conn) {
  ...
  return {
    ...
    antidelete2: !!s.antidelete2,
    antidelete3: normalizeNumber(s.antidelete3),
    // antidelete4 holds a destination GROUP JID, so it is kept raw: a group JID
    // is not a phone number and must not be normalised to digits.
    antidelete4: String(s.antidelete4 || '')
  }
}

function configure(conn, mode, value) {
  const s = sessionSettings(conn)
  // `antidelete1` is the documented name for the original in-chat mode.
  const name = mode === 'antidelete1' ? 'antidelete' : mode
  if (name === 'antidelete') {
    s.antidelete = value === true
  } else if (name === 'antidelete2') {
    s.antidelete2 = value === true
  } else if (name === 'antidelete3') {
    ...
    s.antidelete3 = n
  } else if (name === 'antidelete4') {
    // Set by BIGBRO only after it has resolved and validated the invite link.
    if (!value) return { ok: false, code: 'invalid' }
    s.antidelete4 = String(value)
  } else {
...
function disable4(conn) {
  const s = sessionSettings(conn)
  // Clearing the destination IS what turns antidelete4 off. Nothing is left
  // behind that a later deletion could pick up.
  s.antidelete4 = ''
  saveConfig()
  return getSettings(conn)
}
...
/**
 * Turn EVERY mode on or off at once, persisted.
 *
 * Off also CLEARS the two destinations. That is deliberate: a disabled mode that
 * still has a destination configured is exactly how a feature quietly starts
 * sending again after a restart. Turning it back on therefore requires the
 * destination to be supplied again, which is the honest behaviour.
 */
function setAll(conn, enabled) {
  const s = sessionSettings(conn)
  s.antidelete = enabled === true
  s.antidelete2 = enabled === true
  if (!enabled) {
    s.antidelete3 = ''
    s.antidelete4 = ''
  }
...
async function captureMessage(conn, message) {
  ...
  // Nothing is retained while every mode is off, so a message that arrives
  // before antidelete is switched on cannot be recovered later. That is inherent
  // (you cannot recover what was never stored) and is the honest behaviour.
  if (!settings.antidelete && !settings.antidelete2 && !settings.antidelete3 && !settings.antidelete4) return
...
async function handleDeletion(conn, key) {
  ...
  // Read the persisted settings NOW. If every mode is off the deletion is not
  // even looked up, so a disabled suite cannot send, forward or notify anything.
  const settings = getSettings(conn)
  if (!settings.antidelete && !settings.antidelete2 && !settings.antidelete3 && !settings.antidelete4) return false
  ...
  // antidelete4 archives into the configured destination GROUP. Silent by
  // design: the original chat is never told that a copy was forwarded.
  if (settings.antidelete4) deliveries.push({ dest: settings.antidelete4, mode: 'silent-archive' })
