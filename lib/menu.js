...
/*
 * AI COMMANDS ARE HIDDEN FROM THE MENU.
 *
 * The AI family is large - 38 commands, from `aisticker` and `aivision` to
 * `chatbot`, `darknote` and `autohuman` - and listing them would bury the
 * ordinary commands. They stay fully working: every one is still reachable
 * through the dispatcher and through natural language ("darknote create a
 * sticker of a wolf"). Only the LISTING is suppressed.
 *
 * `^ai` covers the whole ai* family, so a future AI command is hidden
 * automatically rather than appearing in the menu the day it is added.
 *
 * This must stay in step with the AI cases in BIGBRO: the test suite asserts
 * none of them reach the menu, so a miss shows up as a failure rather than as
 * clutter.
 *
 * EXCEPT `autohuman`. It is the one AI feature the owner switches on and off by
 * hand, so it MUST be discoverable - hiding it meant the command worked but
 * nobody could find it, and the feature looked broken. It is therefore not in
 * the hidden set, and the AI category below lists it. `^ai` does not catch it
 * either, because the name starts with "aut".
 */
const HIDDEN_AI_COMMAND = /^ai/i
const HIDDEN_NAMES = new Set([
    'chatbot', 'chatbotdelay', 'replydelay',
    'darknote', 'ask',
    'statusview', 'statuslike', 'statusreact'
])

const isInternal = name => {
    const value = String(name || '')
    if (INTERNAL_COMMAND.test(value)) return true
    return HIDDEN_AI_COMMAND.test(value) || HIDDEN_NAMES.has(value.toLowerCase())
}
 ...
 */
const CATEGORIES = [
    { title: 'MAIN', names: ['menu', 'allmenu', 'ping', 'info', 'owner', 'cekowner', 'myjid', 'public', 'self', 'runtime', 'alive', 'state', 'weather', 'time'] },
    { title: 'AI', names: ['chatbot', 'autohuman', 'aistatus', 'ai', 'darknote', 'aimemory', 'aiforget', 'aimem', 'aireset', 'aiset', 'aicap'] },
    { title: 'GROUP', names: ['tagall', 'hidetag', 'listonline', 'listactive', 'listinactive', 'groupadmins', 'groupadmin', 'admin', 'groupjid', 'channeljid', 'gpstatus', 'gppp', 'kick', 'kickall', 'add', 'promote', 'demote', 'leave', 'join', 'approveall', 'rejectall', 'creategc'] },
